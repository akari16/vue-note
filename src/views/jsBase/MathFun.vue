...
export default {
  ...
  created() {
    /**
     * Math 用于 Number 类型,拥有一些数学常数属性和数学函数方法,Math 不是一个函数对象
     *
     * Math 不是一个构造器。Math 的所有属性与方法都是静态的。引用圆周率的写法是 Math.PI，
     * 调用正余弦函数的写法是 Math.sin(x)，x 是要传入的参数。Math 的常量是使用
     * JavaScript 中的全精度浮点数来定义的。
     *
     * */
